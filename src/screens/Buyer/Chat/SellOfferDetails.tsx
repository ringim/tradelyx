  ...
  const createNotify = async (msgID: string, chatroomID: string) => {
    try {
      const input: CreateNotificationInput = {
        ...
        readAt: 0,
        requestType: 'Sell Offer Reply',
        actorID: userID,
        userID: getSellOfferDetail?.userID,
        SType: 'NOTIFICATION',
        chatroomID: route?.params?.crID,
        notificationSellOfferReplyId: route?.params.sellOffer,
        title: 'Sell Offer Accepted',
        description: `${softData?.getUser?.name} has accepted your Sell Offer request`,
      };
