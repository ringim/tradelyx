...
const EditSellOfferImages = () => {
  const navigation = useNavigation<ProfileStackNavigatorParamList>();
  const route = useRoute<EditSellOfferImagesRouteProp>();
  const {sellOfferID}: any = route?.params;
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      // console.log('sellOffer updated 1', input);
      navigation.goBack();
    } catch (error) {
      Toast.show({
...
export default EditSellOfferImages;
