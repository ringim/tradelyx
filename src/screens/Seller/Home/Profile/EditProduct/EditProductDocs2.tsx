...
const EditProductDoc2 = () => {
  const navigation = useNavigation<ProfileStackNavigatorParamList>();
  const route = useRoute<EditProductDocRouteProp>();
  const {productDoc}: any = route?.params;
  ...
  const onSubmit = async ({file}: ProductData) => {
    if (isSubmitting) {
      return;
    }
    ...
    try {
      ...
      // console.log('product doc updated', input);
      navigation.goBack();
      setSingleFile([]);
    } catch (error) {
      Toast.show({
...
export default EditProductDoc2;
